...
function freqbands = ccs_core_lfobands(samp_numbers, samp_interval, bandwidth,stepwidth)
...
nlcf = nlcfmin:stepwidth:nlcfmax;
numbands = numel(nlcf);
freqbands = cell(numbands,1);
for nlcfID=1:numbands
    [~,idxfmin] = min(abs(freq-exp(nlcf(nlcfID)-bandwidth/2)));    
    [~,idxfmax] = min(abs(freq-exp(nlcf(nlcfID)+bandwidth/2)));
