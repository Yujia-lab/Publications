...
%% Mask file
%%===================================
brainmask = 'D:\program\matlab\Common\Mask\BN_Atlas_246_3mm.nii';
%%===================================
v=spm_vol(brainmask);
brain = spm_read_vols(v);
voxel_ind = find(brain>0); %% change as you like. brain>0 for all brain, or brain==ROIid
...
main= 'F:\lifespan';  % data directory
data_dir = fullfile(main,'FunImgARWC');
save_dir = fullfile(main,'FunImgARWSC_Deconv'); %% save dir, change the name as you like.
...
for isub=1:length(sub)
    ...
    disp('Reading data ...')
    str=['num_',num2str(isub),'_',sub(isub).name];
    disp(str)
    data_now=datestr(now) ;
    disp(data_now);
    
    sub1(isub,1)={
    [data_dir '\' sub(isub).name '\CovRegressed_4DVolume.nii']
    };
    [sub_dir,name,ext] = fileparts(sub1{isub,1});
    sub_save_dir = strrep(sub_dir,main,save_dir);
    mkdir(sub_save_dir)
    
    v1 = spm_vol(sub1{isub,1});
