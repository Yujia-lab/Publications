...
%%% sub_path是被试路径
%%% out_path是输出路径
%%% bands是ccs_core_lfobands算出来的波段，cell数据类型
%%% Fs是频率
...
cd(out_path)
 ...
 for m = 1:length(bands)
    ...
    A = [0 1 0];
    band = bands{m}
    F = [band(1)-0.001, band(1), band(2), band(2)+0.001];
    ...
    out_path2 = fullfile(out_path, ['\slow-', num2str(length(bands)+1-m)]);
    mkdir(out_path2);
     ...
     parfor i = 1:length(sub_file)
        if sub_file(i).isdir == 1
            ...
            cd(brain_path);
            brain = dir('*.nii');
            [brain_data,header]=y_Read(brain.name);% 读取nii
        end
