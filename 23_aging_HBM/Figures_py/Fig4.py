...
"""回归：时间去分化的Hexbin图"""
# 整理数据
Cohere = scio.loadmat('E:\GS Coherence\Regression\ROI_Cohere.mat')
CORR = scio.loadmat('E:\GS Coherence\Regression\Age_GScorr.mat')
spatial_dediff = scio.loadmat('E:\GS Coherence\Regression\spatial_dediff.mat')
...
set_para1()
os.chdir('E:\GS Coherence\FIG_revision\Fig3')
...
"""未回归：时间去分化的Hexbin图"""
# 整理数据
Cohere = scio.loadmat('E:\GS Coherence\Deconv_noreg\ROI_Cohere.mat')
CORR = scio.loadmat('E:\GS Coherence\Deconv_noreg\Age_GScorr.mat')
spatial_dediff = scio.loadmat('E:\GS Coherence\Deconv_noreg\spatial_dediff.mat')
C = np.transpose(Cohere['Mean'])
R = np.transpose(CORR['Z'])
R_spatial = np.squeeze(spatial_dediff['R_tempfdr'])
x = C.reshape((257*246,))
...
# 画图
sns.set(style="ticks")
g = sns.jointplot(x, y, kind="hex", color="b",
            joint_kws={'gridsize':15})
sns.regplot(x, y, ax=g.ax_joint, scatter=False,color='r' )
set_para1()
os.chdir('E:\GS Coherence\FIG_revision\Fig3')
plt.savefig('Nontemp_dediff1.tif',dpi=300)
plt.show()
...
sns.regplot(x2, y2, ax=g.ax_joint, scatter=False,color='r' )
set_para1()
plt.savefig('Nontemp_dediff2.tif',dpi=300)
plt.show()
...
sns.regplot(x3, y3, ax=g.ax_joint, scatter=False,color='r' )
set_para1()
plt.savefig('Nontemp_dediff3.tif',dpi=300)
plt.show()
...
fp = h5py.File('E://GS Coherence//result//f.mat')
R_betw2 = np.squeeze(spatial_dediff['R_spatial'][:])
...
sns.lineplot(f,R_betw2,label='no-regressed data')
sns.lineplot(f,R_betw,label='regressed data')

plt.xlim((0, 0.25))
plt.ylim((-0.65, 0.55))

plt.plot(f[:],np.ones(257,)*0.142,color='r',linewidth=0.5,linestyle='--')
plt.plot(f[:],np.ones(257,)*(-0.142),color='r',linewidth=0.5,linestyle='--')

plt.plot(f[:28],np.ones(28,)*0.45,color='black',linewidth=2)
plt.plot(f[61:82],np.ones(21,)*(0.45),color='black',linewidth=2)
plt.plot(f[135:],np.ones(122,)*(0.45),color='black',linewidth=2)

my_xticks1=np.arange(0,0.26,0.05)
plt.xticks(my_xticks1,xtick,fontname='Arial',fontsize=tick_size)
plt.yticks([-0.6,-0.4,-0.2,0,0.2,0.4],fontname='Arial',fontsize=tick_size)
...
plt.legend(['regressed data','no-regressed data'],
           prop={'family':'Arial','size':7})

os.chdir('E:\GS Coherence\FIG_revision\Fig3')
plt.savefig('Nonspatial_dediff1.tif',dpi=300)
plt.show()
...
plt.xlim(0.22,0.64)
plt.ylim(-0.26,0.17)
my_xticks=np.arange(0.3,0.61,0.1)

set_para2()

plt.savefig('Nonspatial_dediff2.tif',dpi=300)
plt.show()
...
set_para2()
plt.savefig('Nonspatial_dediff3.tif',dpi=300)
plt.show()
...
fig = plt.figure(figsize=(cm2inch(5.5), cm2inch(4.5)))
h = plt.axes([0.18,0.16,0.77,0.8])
sns.lineplot(f,R_betw2,label='no-deconvolved data')
sns.lineplot(f,R_betw,label='deconvolved data')
plt.xlim((0, 0.25))
plt.plot(f[:],np.ones(257,)*0.134,color='r',linewidth=0.5,linestyle='--')
plt.plot(f[:],np.ones(257,)*(-0.134),color='r',linewidth=0.5,linestyle='--')
my_xticks1=np.arange(0,0.26,0.05)
plt.xticks(my_xticks1,xtick,fontname='Arial',fontsize=tick_size)
plt.yticks([-0.6,-0.4,-0.2,0,0.2],fontname='Arial',fontsize=tick_size)
plt.xlabel('Frequency (Hz)',fontname='Arial',fontsize=label_size,labelpad=1)
plt.ylabel('R',fontname='Arial',fontsize=label_size,labelpad=1)
plt.tick_params(axis='both',length=3,pad=1)

plt.legend(['deconvolved data','no-deconvolved data'],
           prop={'family':'Arial','size':7})
