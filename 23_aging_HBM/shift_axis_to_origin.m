...
linewid=get(gca,'linewidth');

set(gca,'XTick',[],'XColor','w') ;
set(gca,'YTick',[],'YColor','w') ;

% 把 x 和 y 坐标轴的两个方向各延长 10% （为了视觉上好看）
extend_x = ( xL(2)-xL(1) ) * 0 ;
extend_y = ( yL(2)-yL(1) ) * 0 ;
...
xlim(xxL) ;
box off ;
set(temp_1,'XTick',xt,'XTicklabel',xtl,'Color','None','YTick',[],'linewidth',linewid) ;
set(temp_1,'YColor','w') ;

temp_2 = axes( 'Position', pos + [ pos(3) * y_shift , 0 , -pos(3)* y_shift*0.99999 , 0 ] ) ;
ylim(yyL) ;
box off ;
set(temp_2,'YTick',yt,'YTicklabel',ytl,'Color','None','XTick',[],'linewidth',linewid) ;
set(temp_2,'XColor','w') ;
